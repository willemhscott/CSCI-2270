...
Node *BST::deleteNode(Node *currNode, int value) {
    ...
    if (currNode == NULL) {
        ...
        if (currNode->left == NULL && currNode->right == NULL) {
            ...
            //Node *prev = currNode;
            Node *predec = currNode->left;
            while (predec->right != NULL) {
                //prev = predec;
                predec = predec->right;
            }
            //prev->right = NULL;
            currNode->key = predec->key;
            //cout << "removing p: " << predec->key << endl;
            currNode->left = deleteNode(currNode->left, predec->key);
        }
...
bool verifyChildren(Node *root) {
    bool value = true;
    if (root->left != NULL) {
        value = value && root->left->key < root->key && verifyChildren(root->left);
    }

    if (root->right != NULL) {
        value = value && root->right->key > root->key && verifyChildren(root->right);
    }
    return value;
